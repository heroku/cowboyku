...
handle(Req, State) ->
	true = cowboyku_req:has_body(Req),
	{ok, Req3} = case cowboyku_req:body(1000000, Req) of
...
handle_chunked(Req) ->
	...
	{ok, Req3} = cowboyku_req:reply(200, [], Data, Req2),
	{ok, Req3}.

handle_badlength(Req) ->
	{ok, Req2} = cowboyku_req:reply(413, [], <<"Request entity too large">>, Req),
	{ok, Req2}.

handle_body(Req, Body) ->
	{Size, Req2} = cowboyku_req:body_length(Req),
	Size = byte_size(Body),
	{ok, Req3} = cowboyku_req:reply(200, [], Body, Req2),
	{ok, Req3}.

...
handle(Req, State) ->
	...
	{ok, Req3} = cowboyku_req:reply(200, [], term_to_binary(Result), Req2),
	{ok, Req3, State}.
...
acc_multipart(Req) ->
	acc_multipart(cowboyku_req:multipart_data(Req), []).

acc_multipart({headers, Headers, Req}, Acc) ->
	acc_multipart(cowboyku_req:multipart_data(Req), [{Headers, []}|Acc]);
acc_multipart({body, Data, Req}, [{Headers, BodyAcc}|Acc]) ->
	acc_multipart(cowboyku_req:multipart_data(Req), [{Headers, [Data|BodyAcc]}|Acc]);
